...
run_matrix() {
    ...
    docker rm -f "$container" >/dev/null
    python3 -c "import json, sys; results = json.load(sys.stdin); assert len(results) == 23, \"{} matrix cases executed; expected 23\".format(len(results)); print(\"{} matrix: {} passed\".format(sys.argv[1], len(results)))" "$label" <<<"$result"
}
